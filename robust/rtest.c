...
// External parameters for the qsort call:
...
// for sorting in the test_p_value function
...
bool test_p_value(long double pvalue[], // here the array of p-value is returned 
      ...
      bool output, char *output1, char *output2, // whether output is needed and where 
      bool debug, // whether debug information should be printed 
  ...
  for (coord=0; coord<dimension; coord++){
    ...
    bool kstring[n0+n1]; // source of ith bits (false/true if from ref0/ref1 array)
    int pos0= 0;
    int pos1= 0;
...
#define SAMPLE_SIZE_BOUND 10000
  ...
  while(ok){
    ...
    ok= test_p_value (p, functions_list[num_func].reference, test, compare_etalon, int_parameters, real_parameters, output, output1, output2, debug, ksexact);
    if (ok){
